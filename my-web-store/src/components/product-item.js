...
// Tope de espera del bulk-inventory. La promesa global la crea cada página, pero no
// todas la resuelven; sin este tope una tarjeta se quedaría fija en "Consultando...".
const ESPERA_BULK_MS = 2500;
  ...
  return /* html */`
    <article class="v2-card reveal" data-id="${p.id}" data-sku="${skuAttr}" onclick="if(!event.target.closest('button') && !event.target.closest('input')) { window.location.href='/product?id=${p.id}'; }">
      ...
      <div class="v2-card-actions">
        ...
        <button class="v2-add-btn add-to-cart" data-id="${p.id}"${skuAttr ? ' disabled data-verificando="1" title="Consultando inventario..."' : ''}>
...
export function attachDynamicPriceBehavior(rootEl) {
  ...
  if (skuAttr) {
    ...
    const checkStock = async () => {
      try {
        // Se espera la consulta masiva de la página en vez de pedir una por tarjeta,
        // pero con un tope: hay páginas que crean la promesa y nunca la resuelven.
        if (window._inventoryReady) {
          try {
            await Promise.race([
              window._inventoryReady,
              new Promise(resolve => setTimeout(resolve, ESPERA_BULK_MS))
            ]);
          } catch { /* se cae a la consulta individual */ }
        }
        ...
        try {
          ...
          // Si no se pudo consultar, no se deja el botón colgado en "Consultando...".
          applyOutOfStock();
        }
      } catch (e) {
