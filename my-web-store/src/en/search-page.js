...
function getCatLabel(p) {
  const raw = String(p.category_name || p.category_nombre || p.category_desc || p.category || 'General').trim();
  const rawL = raw.toLowerCase();
  if (rawL.includes('vaso')) return 'Cups';
  if (rawL.includes('tapa')) return 'Lids';
  if (rawL.includes('contenedor')) return 'Food Tubs';
  if (rawL.includes('porta vaso') || rawL.includes('portavaso')) return 'Cup Holders';
  if (rawL.includes('empaque')) return 'Packaging';
  return raw;
}
...
async function init() {
  ...
  document.title = query ? `"${query}" — Search KosXpress` : 'Search — KosXpress';

  const resProd = await fetch('/api/products');
  const products = resProd.ok ? withEnglishCopyList(await resProd.json()) : [];
  ...
  function renderStats(filtered, q) {
    ...
    statsEl.innerHTML = `
      <span class="stats-badge"><span id="anim-count">0</span>&nbsp;result${filtered.length !== 1 ? 's' : ''}</span>
      ${catCount > 1 ? `<span class="stats-sep">·</span><span class="stats-cats">${catCount} categories</span>` : ''}
      ${q.trim() ? `<span class="stats-sep">·</span><span class="stats-for">for <em>"${q}"</em></span>` : ''}
    `;
    ...
    if (list.length === 0) {
      resultsEl.innerHTML = `
        <div class="search-empty-state">
          ...
          <h3>No results</h3>
          <p>We couldn't find any products matching <strong>"${q}"</strong>.<br>Try another term or browse our catalog.</p>
          <a href="/en/products" class="btn-primary" style="display:inline-block;margin-top:24px;padding:12px 36px;text-decoration:none;border-radius:14px;">
            View all products
          </a>
        </div>
      `;
  ...
  function run(q) {
    ...
    document.title = q ? `"${q}" — Search KosXpress` : 'Search — KosXpress';
  }
  ...
  resultsEl.addEventListener('click', async (e) => {
    ...
    try {
      ...
      if (estado !== 'En Existencia') {
        showToast('Product Out of Stock', 'error'); return;
      }
      ...
      showToast('Added Successfully');
      setTimeout(() => btn.classList.remove('added'), 350);
    } catch { showToast('Product Out of Stock', 'error'); }
