export function initChatbot() {
  ...
  container.innerHTML = `
    ...
    <div class="kos-welcome-bubble" id="kos-welcome-bubble">
      Hi, I'm Koski, your personal assistant
    </div>
      ...
      <div class="kos-chatbot-header">
        <div class="kos-chatbot-header-info">
          ...
          <div class="kos-chatbot-header-text">
            ...
            <span class="kos-chatbot-header-status"><span class="kos-status-dot"></span> Online</span>
          </div>
        </div>
        <button class="kos-chatbot-close">&#x2715;</button>
      </div>
      <div class="kos-chatbot-messages" id="chatbot-msg-container">
        <div class="chatbot-msg bot">Hi! I'm Koski, your AI assistant. I'm here to help with anything you need.</div>
        <div class="kos-chatbot-suggestions" id="chatbot-suggestions">
          <button class="kos-suggestion-chip" data-msg="I'd like a wholesale quote">📦 Wholesale quote</button>
          <button class="kos-suggestion-chip" data-msg="I need to speak with a human advisor">👤 Talk to an advisor</button>
          <button class="kos-suggestion-chip" data-msg="I want to track my order">📋 Track my order</button>
          <button class="kos-suggestion-chip" data-msg="I need information about your products">ℹ️ Product info</button>
        </div>
    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          ...
          if (!replyText) {
            appendError("Sorry, I couldn't process that response. Please try again.", text);
            return;
          }
        } catch {
          replyText = textResponse && textResponse.trim() ? textResponse : null;
          if (!replyText) {
            appendError('I received an empty response. Please try again.', text);
            return;
          }
        }
        appendMessage(replyText, 'bot');
      } else {
        appendError('There was a problem processing your request. Please try again in a moment.', text);
      }
      ...
      showTyping(false);
      setInputEnabled(true);
      appendError("I couldn't connect to the server. Check your connection and try again.", text);
      input.focus();
    }
  };
