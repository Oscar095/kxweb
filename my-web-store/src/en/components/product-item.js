...
export function productItemTemplate(p) {
  ...
  return /* html */`
    <article class="v2-card reveal" data-id="${p.id}" data-sku="${skuAttr}" onclick="if(!event.target.closest('button') && !event.target.closest('input')) { window.location.href='/en/product?id=${p.id}'; }">
      <div class="v2-card-img-wrap">
        <div class="v2-oos-badge" style="display: none;">OUT OF STOCK</div>
      ...
      <div class="v2-card-body">
        ...
        <div class="v2-card-price">
          <p class="price" data-base-price="${unitPrice}" data-cantidad="${cantidadNum ?? ''}" data-codigo="${skuAttr}">
            ...
            <span class="price-unit">${cantidadNum ? '/ ' + new Intl.NumberFormat('en-US').format(cantidadNum) + ' pcs box' : '/ box'}</span>
            <span class="iva-tag">VAT included</span>
          </p>
        </div>
      </div>
...
export function attachDynamicPriceBehavior(rootEl) {
  ...
  if (skuAttr) {
    ...
    const checkStock = async () => {
      try {
        ...
        console.warn('[checkStock] Unexpected error:', e);
      }
    };
    checkStock();
  }
  ...
  function renderUnitBoxPrice(data, fallbackBase) {
    ...
    const labelTexto = hasCantidad ? '/ ' + new Intl.NumberFormat('en-US').format(BOX_SIZE) + ' pcs box' : '/ box';
    priceEl.innerHTML = '$' + fmt(precioConIva) + ` <span class="price-unit">${labelTexto}</span> <span class="iva-tag">VAT included</span>`;
