...
async function showPromoPopup() {
  ...
  const pctBadge = (bono && bono.porcentaje_descuento != null)
    ...
    : '';
  // Urgency: if the bono has a closing date, show it to reinforce the promo (also
  // useful when the popup content gets shared on social media).
  const vigenciaTexto = (bono && bono.fecha_fin)
    ? `<p class="promo-v3-vigencia">Valid until ${new Date(bono.fecha_fin).toLocaleDateString('en-US', { day: '2-digit', month: '2-digit', year: 'numeric' })}</p>`
    : '';
